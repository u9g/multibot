...
function renderCommand(bot, page) {
  return new Promise((resolve, reject) => {
    ...
    bot.chat(`/level top ${page}`);

    bot.on('message', (msg) => {
      const ft = msg.toString();
      if (regex.start.test(ft)) showingPeople = true;
...
function createEmbed(info, page, timePassed) {
  ...
  const title =
    page > 1
      ? `Top Player Levels (${page} / 20)`
      : 'Top Player Levels (1 / 20)';
...
const createDescription = (info) => {
  const desc = info.map((user, ix) => {
    let [ign, lvl] = user;
    return `${lvl} => \`${ign}\``;
    //return `${ix + 1}. **${ign}** has player level **${lvl}**`;
  });
  return desc;
};
